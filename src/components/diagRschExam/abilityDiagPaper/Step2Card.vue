...
/** 역량 이름 설정하기 */
const getAbilityCategoryLabel = () => {
  switch (paper.value.dapAbilityCategory) {
    case "C":
      return "핵심역량";
    case "M":
      return "전공능력";
...
const onClickDownloadExcelForm = () => {
  const label = getAbilityCategoryLabel();

  excelDown(headers, items, label + "진단_문항_양식");
};
...
const getAbilityEnum = () => {
  const param = {
    cdDiv: paper.value.dapAbilityCategory,
  };
  vm.$axios
...
const headers: Header[] = [
  {
    header: getAbilityCategoryLabel() + "코드",
    key: "upAbilityCd",
    width: 20,
    required: true,
  },
