<template>
  ...
  <v-row>
    <v-col>
      <v-row v-for="(ansInfo, idx) in model.ansInfo" :key="idx">
        <v-col cols="12" md="2" class="d-flex align-center text-h6">
          <span class="required" v-if="idx === 0">보기 내용 입력</span>
        </v-col>
        <v-col cols="12" md="10" class="d-flex align-center py-1">
          <span class="d-flex align-center ml-3 text-h6">{{ ansInfo.order + 1 }}</span>
          <v-text-field class="ml-3 px-2" v-model="ansInfo.title" placeholder="보기내용을 입력해주세요."></v-text-field>
...
onMounted(() => {
  ...
  model.value.ansInfo = defaultScale;
});
