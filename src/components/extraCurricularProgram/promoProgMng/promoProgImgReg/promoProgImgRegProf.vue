    ...
    <v-col cols="12">
      ...
      <div class="program_thumbbox big mx-auto">
        ...
        <p :class="mainImgClass">
          <input type="file" @change="fnGetProfFileName($event, 0, 'mainProfImg')" class="d-none" id="uptProfImg" />
          <label for="uptProfImg"> 메인 배너 홍보이미지를 등록해주세요 </label>
        </p>
        <div class="img_wrap">
          <img id="mainProfImg0" :src="mainImg" alt="대학 로고이미지 예시" />
          <div>
            <v-btn
              icon
              size="small"
              variant="tonal"
              @click="onClickDeleteSrc('mainProf', 0)"
              class="attach_delete"
      ...
      <div class="program_grid mx-auto">
        <div class="program_thumbbox" v-for="(sub, index) in subImg" :key="index">
          ...
          <p :class="subImgClass[index]">
            <input
              type="file"
              @change="fnGetProfFileName($event, index, 'subProfImg')"
              class="d-none"
              :id="`upSubProfImg${index}`"
            />
            <label v-if="subImgClass[index] == ''" :for="`upSubProfImg${index}`"></label>
            <label v-else :for="`upSubProfImg${index}`">서브 배너 홍보이미지를 등록해주세요</label>
          </p>
          <div class="img_wrap">
            <img :src="subImg[index]" :id="`subProfImg${index}`" />
            <v-btn
              icon
              size="small"
              variant="tonal"
              @click="onClickDeleteSrc('subProf', index)"
              class="attach_delete"
              color="bgTransparency"
              ><v-icon>mdi-window-close</v-icon></v-btn
            >
...
export default {
  ...
  methods: {
    //이미지 등록
    fnGetProfFileName(event: Event, num: number, menuType: string) {
    ...
    onClickDeleteSrc(type: string, num: number) {
      if (type === `mainProf`) {
        let mainId: any = document.getElementById(`mainProfImg0`);
        mainId.src = "/assets/images/program/img_program_thumbnail.png";
        this.mainImgClass = "attach_img";
      } else {
        let subImg: any = document.getElementById(`subProfImg${num}`);
        subImg.src = "";
    ...
    fnCallMount() {
      ...
      this.subImgClass = ["", "", "", ""];
      this.subImg = [
        "/src/assets/images/program/img_program_thumbnail.png",
      ...
      this.mainImgContent = {
        title: "메인 이미지 타이틀 예시입니다(교수)",
        regDate: "2024-01-02 ~ 2024-01-31(교수)",
        orptDate: "2024-02-02 ~ 2024-02-31(교수)",
        ability: "종합적 사고력ㆍ대인관계0(교수)",
        target: "재학생(교수)",
      };
      this.subImgContent = {
        title: [
          "서브 이미지 타이틀 예시입니다(교수)1",
          "서브 이미지 타이틀 예시입니다(교수)2",
          "서브 이미지 타이틀 예시입니다(교수)3",
          "서브 이미지 타이틀 예시입니다(교수)4",
        ],
        ability: [
          "종합적 사고력ㆍ대인관계1(교수)",
          "종합적 사고력ㆍ대인관계2(교수)",
        ...
        regDate: [
          "2024-01-12 ~ 2024-01-31(교수)",
          "2024-01-02 ~ 2024-01-31(교수)",
          "2024-03-02 ~ 2024-03-31(교수)",
          "2024-04-02 ~ 2024-04-31(교수)",
        ],
        orptDate: [
          "2024-02-12 ~ 2024-02-31(교수)",
          "2024-03-02 ~ 2024-03-31(교수)",
          "2024-04-02 ~ 2024-04-31(교수)",
          "2024-05-02 ~ 2024-05-31(교수)",
        ],
        target: ["재학생(교수)1", "재학생2(교수)", "재학생3(교수)", "재학생4(교수)"],
      };
    },
  },
};
