  ...
  methods: {
    ...
    onClickCancel() {
      ...
      vm.$confirm("취소하시겠습니까?", "", (isConfirm: boolean) => {
        if (isConfirm) {
          vm.$router.push("/admin/diagRschExam/coreAbilityDiag/coreDiagPaperReg");
        }
      });
    },
    ...
    onClickSave() {
      ...
      if (vm.viewType === "REG") {
        alertMsg = "저장";
        url = "/api/admin/diag/coreAbility/createCoreDiagPaper";
      } else if (vm.viewType === "MODIFY") {
        alertMsg = "수정";
        url = "/api/admin/diag/coreAbility/modifyCoreDiagPaper";
      }
