        ...
        <v-col>
          <BaseCard>
            <template v-slot:title>
              <div class="d-flex align-center">
                <div class="text-h4">핵심역량진단 실시계획 목록</div>
                <!-- tooltip -->
...
export default {
  ...
  methods: {
    ...
    fnGetDiagPlanList() {
      ...
      vm.loading = true;
      vm.$axios
        .get("/api/admin/diag/coreAbility/getCoreDiagPlan", { params: param })
    ...
    onClickChangeState(state: String) {
      ...
      vm.$confirm(`실시계획을 ${msg}하시겠습니까?`, "확인", (isConfirm: boolean) => {
        if (isConfirm) {
          ...
          vm.$loading.show();
          vm.$axios
            .post("/api/admin/diag/coreAbility/modifyCorePlanState", JSON.stringify(param))
            .then((res: AxiosResponse) => {
