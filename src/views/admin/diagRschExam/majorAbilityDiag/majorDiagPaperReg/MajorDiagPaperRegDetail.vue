            ...
            <v-row>
              <v-col>
                <LabelCard :title="'STEP 1 척도 수 입력'">
                  <Step1Card v-model="diagPaperInfo.dapAnsCnt" :disabled="activeRow[0] || viewType == 'MODIFY'">
                  </Step1Card>
                </LabelCard>
                <v-row>
                  <v-col cols="12">
  ...
  methods: {
    ...
    onClickCancel() {
      ...
      vm.$confirm("취소하시겠습니까?", "", (isConfirm: boolean) => {
        if (isConfirm) {
          vm.$router.push("/admin/diagRschExam/majorAbilityDiag/majorDiagPaperReg");
        }
      });
    },
    ...
    onClickSave() {
      ...
      if (vm.viewType === "REG") {
        alertMsg = "저장";
        url = "/api/admin/diag/majorAbility/createMajorDiagPaper";
      } else if (vm.viewType === "MODIFY") {
        alertMsg = "수정";
        url = "/api/admin/diag/majorAbility/modifyMajorDiagPaper";
      }
