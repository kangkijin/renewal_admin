        ...
        <v-col>
          <BaseCard>
            <template v-slot:title>
              <div class="d-flex align-center">
                <div class="text-h4">교수역량 진단지 목록</div>
                <!-- tooltip -->
...
export default {
  ...
  methods: {
    ...
    fnGetDiagPaperList() {
      ...
      vm.loading = true;
      vm.$axios
        .get("/api/admin/diag/profAbility/getProfDiagPaper", { params: param })
      ...
      if (vm.selectedItems.length === 0) {
        ...
        vm.$confirm("진단지를 복사하시겠습니까?", "확인", (isConfirm: Boolean) => {
          if (isConfirm) {
            ...
            vm.$loading.show();
            vm.$axios
              .post("/api/admin/diag/profAbility/createCopyProfDiagPaper", JSON.stringify(param))
              .then((res: AxiosResponse) => {
    ...
    onClickChangeState(state: String) {
      ...
      vm.$confirm(msg, "확인", (isConfirm: Boolean) => {
        if (isConfirm) {
          ...
          vm.$loading.show();
          vm.$axios
            .post("/api/admin/diag/profAbility/modifyProfDiagState", JSON.stringify(param))
            .then((res: AxiosResponse) => {
              const resData = res.data;
