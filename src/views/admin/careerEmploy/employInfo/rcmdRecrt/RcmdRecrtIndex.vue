    ...
    <v-col>
      <BaseCard class="mb-6">
        <template v-slot:title>
          <div class="text-h6">추천 채용 현황</div>
        </template>
        <v-row>
          <v-col cols="3">
            <div class="d-flex align-center justify-center">
              <p class="text-subtitle-1 font-weight-bold">채용공고 수</p>
              <p class="text-h4 font-weight-bold ml-5">{{ cntList.totalRecrtCnt }}</p>
            </div>
          </v-col>
          <v-col cols="3">
            <div class="d-flex align-center justify-center">
              <p class="text-subtitle-1 font-weight-bold">지원인원</p>
              <p class="text-h4 text-success font-weight-bold ml-5">{{ cntList.applyCnt }}</p>
            </div>
          </v-col>
          <v-col cols="3">
            <div class="d-flex align-center justify-center">
              <p class="text-subtitle-1 font-weight-bold">추천인원</p>
              <p class="text-h4 text-warning ml-5">{{ cntList.rcmdCnt }}</p>
            </div>
          </v-col>
          <v-col cols="3">
            <div class="d-flex align-center justify-center">
              <p class="text-subtitle-1 font-weight-bold">입사인원</p>
              <p class="text-h4 text-info font-weight-bold ml-5">{{ cntList.joinCnt }}</p>
            </div>
          </v-col>
        </v-row>
      </BaseCard>
      <BaseCard>
        ...
        >
          ...
          <template #item-operation2="item">
            <v-btn color="warning" size="small" @click="onClickRecrtApplyTarget(item.recrtId)"
              >{{ item.rcmdCnt }}명</v-btn
            >
          </template>
          <template #item-operation3="item">
            <v-btn color="primary" size="small" @click="onClickRecrtApplyTarget(item.recrtId)"
              >{{ item.joinCnt }}명</v-btn
            >
          </template>
          <template #item-operation4="item">
            <v-dialog v-model="dialog" persistent width="1024">
              <template v-slot:activator="{ props }">
                <v-btn size="small" v-bind="props" @click="onClickShowRsltPopup(item.recrtId)">추가</v-btn>
              </template>
              <AddUserPopup
                v-model="dialog"
                :recrtId="selectedKey"
                :recrtTypeCd="searchInfo.recrtTypeCd"
                @update:completeAddUser="fnGetRecrtList"
              />
            </v-dialog>
          </template>
...
import AddUserPopup from "@/components/employInfo/AddUserPopup.vue";
...
export default {
  ...
  data: () => ({
    ...
    header: [
      ...
      { text: "채용공고명", value: "recrtTitle", sortable: true, width: "auto" },
      { text: "모집분야", value: "recrtField" },
      { text: "모집기간", value: "recrtApplyDate", width: "auto" },
      { text: "지원인원", value: "operation1" },
      { text: "추천인원", value: "operation2" },
      { text: "입사인원", value: "operation3" },
      { text: "학생추가", value: "operation4" },
  ...
  methods: {
    ...
    fnGetRecrtCntList() {
      ...
      vm.$loading.show();
      vm.$axios
        .get("/api/career/admin/recrtInfo/getRecrtInfoCntInfo", { params: searchParam })
        .then((res: AxiosResponse) => {
          vm.cntList = res.data.data;
        })
        .catch((e: any) => {
          console.error(e);
        })
    ...
    onClickShowRsltPopup(_keyId: string) {
      const vm = this;
    ...
    onClickSearchReset() {
      ...
      vm.searchInfo.recrtTypeCd = "REC_TYPE01";
    },
    ...
    fnExcelDown() {
      ...
      const headers: Header[] = [
        ...
        { header: "추천인원", key: "rcmdCnt", width: 10 },
        { header: "입사인원", key: "joinCnt", width: 10 },
      ];
