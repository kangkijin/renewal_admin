...
export default {
  data: () => ({
    ...
    breadcrumbs: [
      ...
      { title: "Foreign Worker", disabled: true },
    ],
    /** ck에디터 */
  ...
  computed: {
    title() {
      const vm = this;
      return `Foreign Worker ${vm.viewType === "MODIFY" ? "수정" : "등록"}`;
    },
    saveButtonText() {
              ...
              .then((res: AxiosResponse) => {
                ...
                if (code === "200") {
                  vm.$alert(text + "되었습니다.", "알림", () => {
                    vm.$router.push("/admin/careerEmploy/employInfo/foreignWorker");
                  });
                }
              })
              .catch((e: any) => {
