      ...
      <BaseCard>
        ...
        >
          <template #top-btn-area>
            <v-btn class="mr-2" to="/admin/extraCurricularProgram/userEcp/userCurriReg">과정 등록</v-btn>
        ...
        >
          <template #item-operation="item">
            <v-btn
              @click.stop
              size="small"
              :to="{ path: '/admin/extraCurricularProgram/userEcp/userCurriModify', query: { eciId: item.eciId } }"
            >
              수정</v-btn
            >
...
export default {
  ...
  methods: {
    ...
    async fnUpdateCurriBtn(updUseYn: String) {
      ...
      await vm.$axios.post("/api/admin/ecpUser/updateUseYnEcpCurri", JSON.stringify(updParams));
      vm.fnGetCurriList();
      vm.selectedItems = [];
    },
    ...
    async fnDeleteEcpCurri() {
      if (confirm("선택된 과정을 삭제하시겠습니까?")) {
        ...
        await vm.$axios.post("/api/admin/ecpUser/deleteEcpCurri", JSON.stringify(delParams));
        alert("삭제되었습니다.");
        vm.fnGetCurriList();
        vm.selectedItems = [];
      }
    ...
    async fnCopyEcpCurri() {
      if (confirm("선택된 과정을 복사하시겠습니까?")) {
        ...
        await vm.$axios.post("/api/admin/ecpUser/copyEcpCurri", JSON.stringify(copyParams));
        vm.fnGetCurriList();
        vm.selectedItems = [];
      }
    },
