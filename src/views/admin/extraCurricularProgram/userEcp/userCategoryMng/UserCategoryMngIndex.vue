      ...
      <BaseCard>
        ...
        >
          <template #top-btn-area>
            <!-- 카테고리 등록버튼 -->
            <UserCategoryRegPop @dataModified="fnGetCategoryList" />
          </template>
        ...
        >
          <template #item-operation="item">
            <!-- 카테고리 수정버튼 -->
            <UserCategoryModifyPop :tableItem="item" @dataModified="fnGetCategoryList" />
          </template>
        </v-easy-table>
      </BaseCard>
    </v-col>
...
import UserCategoryRegPop from "./popup/UserCategoryRegPopup.vue";
import UserCategoryModifyPop from "./popup/UserCategoryModifyPopup.vue";
import type { TableServerOption } from "@/types/dataTable/EasyDataTableConfig";

export default {
  components: {
    UserCategoryRegPop,
    UserCategoryModifyPop,
  },
  ...
  methods: {
    ...
    fnGetCategoryList() {
      ...
      vm.$axios
        .get("/api/admin/ecpUser/ecpCategorys", { params: params })
    ...
    async fnUpdateCategory(updUseYn: String) {
      const vm = this;
      if (updUseYn === "N") {
        var ecmCntYn = this.selectedItems.some((x: { ecmCnt: number }) => x.ecmCnt > 0);
        console.log(ecmCntYn);
        if (ecmCntYn) {
          alert("현재 사용중인 과정이 존재하는 카테고리입니다.");
          return false;
        }
      }
      ...
      await vm.$axios.post("/api/admin/ecpUser/updateUseYnEcpCategorys", JSON.stringify(updParams));
      vm.fnGetCategoryList();
      vm.selectedItems = [];
    },
    //카테고리 삭제 버튼 <!--TODO 아직 사용 여부 확인 후 삭제 기능은 없음.-->
    async fnDeleteEcpCategorys() {
      var ecmCntYn = this.selectedItems.some((x: { ecmCnt: string }) => parseFloat(x.ecmCnt) > 0);
      if (ecmCntYn) {
        alert("현재 사용중인 과정이 존재하는 카테고리입니다.");
        return false;
      }
      if (confirm("선택된 카테고리를 삭제하시겠습니까?")) {
        ...
        await vm.$axios.post("/api/admin/ecpUser/deleteEcpCategorys", JSON.stringify(delParams));
        vm.fnGetCategoryList();
        vm.selectedItems = [];
      }
    },
