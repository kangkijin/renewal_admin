      ...
      <BaseCard>
        ...
        >
          <template #top-btn-area>
            <!-- 카테고리 등록버튼 -->
            <OtsdrCategoryRegPop @dataModified="fnGetCategoryList" />
          </template>
        ...
        >
          <template #item-operation="item">
            <!-- 카테고리 수정버튼 -->
            <OtsdrCategoryModifyPop :tableItem="item" @dataModified="fnGetCategoryList" />
          </template>
        </v-easy-table>
      </BaseCard>
    </v-col>
...
import OtsdrCategoryRegPop from "./popup/OtsdrCategoryRegPopup.vue";
import OtsdrCategoryModifyPop from "./popup/OtsdrCategoryModifyPopup.vue";
import type { TableServerOption } from "@/types/dataTable/EasyDataTableConfig";

export default {
  components: {
    OtsdrCategoryRegPop,
    OtsdrCategoryModifyPop,
  },
  data: () => ({
    title: "카테고리 관리",
    breadcrumbs: [
  ...
  methods: {
    ...
    fnGetCategoryList() {
      ...
      vm.$axios
        .get("/api/admin/ecpOtsdr/ecpCategorys", { params: params })
    ...
    async fnUpdateCategory(updUseYn: String) {
      ...
      await vm.$axios.post("/api/admin/ecpOtsdr/updateUseYnEcpCategorys", JSON.stringify(updParams));
    ...
    async fnDeleteEcpCategorys() {
      if (confirm("선택된 카테고리를 삭제하시겠습니까?")) {
        ...
        await vm.$axios.post("/api/admin/ecpOtsdr/deleteEcpCategorys", JSON.stringify(delParams));
        vm.fnGetCategoryList();
        vm.selectedItems = [];
      }
    },
