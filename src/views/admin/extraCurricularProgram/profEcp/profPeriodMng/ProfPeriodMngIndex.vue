      ...
      <BaseCard>
        ...
        >
          <template #top-btn-area>
            <v-btn class="mr-2" to="/admin/extraCurricularProgram/profEcp/profPeriodReg">차수 등록</v-btn>
        ...
        >
          <template #item-operation="item">
            <div id="btn-area">
              <v-btn
                size="small"
                :to="{ path: '/admin/extraCurricularProgram/profEcp/profPeriodModify', query: { eliId: item.eliId } }"
                @click.stop
                >수정</v-btn
              >
              <v-btn
...
export default {
  ...
  methods: {
    ...
    async fnUpdatePeriodBtn(updUseYn: String) {
      ...
      await vm.$axios.post("/api/admin/ecpProf/updateUseYnEcpPeriod", JSON.stringify(updParams));
      vm.fnGetPeriodList();
      vm.selectedItems = [];
    },
    ...
    async fnDeleteEcpPeriod() {
      if (confirm("선택된 차수를 삭제하시겠습니까?")) {
        ...
        await vm.$axios.post("/api/admin/ecpProf/deleteEcpPeriod", JSON.stringify(delParams));
        alert("삭제되었습니다.");
        vm.fnGetPeriodList();
        vm.selectedItems = [];
      }
    ...
    async fnCopyEcpPeriod() {
      if (confirm("선택된 차수를 복사하시겠습니까?")) {
        ...
        await vm.$axios.post("/api/admin/ecpProf/CopyEcpPeriod", JSON.stringify(copyParams));
        vm.fnGetPeriodList();
        vm.selectedItems = [];
      }
    },
    ...
    fnGetPeriodList() {
      ...
      vm.$axios
        .get("/api/admin/ecpProf/ecpPeriodList", { params: searchParam })
    ...
    rejectApply(eliId?: string) {
      ...
      vm.$axios
        .get("/api/admin/ecpProf/getEcpRejectHis", { params: param })
        .then((res: AxiosResponse) => {
